...
wlanip() {
  WLANIP=$(ifconfig iwx0 | grep inet | grep iwx0 | awk '{split($2,a,"%")}; {print a[1]}')
  [[ -n $WLANIP ]] && echo -n " $WLANIP"
}

wifi() {
  WIFI=$(ifconfig iwx0 | egrep ieee80211 | awk '{print $8}' | sed 's/\%//' )
  SSID=$(ifconfig iwx0 | egrep ieee80211 | awk '{print $3}')
  if [ $WIFI -lt 21 ]; then
    echo -n "${SSID} "
